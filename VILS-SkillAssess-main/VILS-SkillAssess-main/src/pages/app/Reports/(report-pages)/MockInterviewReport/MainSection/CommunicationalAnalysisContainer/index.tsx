...
const CommunicationalAnalysisContainer = () => {
    const { currentQuestionData, getWPMLabel } = useMockInterviewReportsContext();
    if (!currentQuestionData || currentQuestionData?.skipped) return null;

    const wpmScore = getWPMLabel(
        currentQuestionData?.result?.communication_analysis?.['words_per_minute']['value'],
    );
            ...
            <div className="border border-gray-300 rounded-md overflow-hidden">
                ...
                <div className="flex">
                    <div className="w-1/3 border-r-[0.5px] border-gray-300 py-6 grid place-content-center">
                        ...
                        <p className="text-center text-sm mt-2">{currentQuestionData?.result?.communication_analysis?.number_of_pauses[
                                    'category'
                                ]}</p>
                    ...
                    <div className="w-1/3 border-l-[0.5px] border-gray-300 py-6 grid place-content-center">
                        <p className="text-primary text-4xl font-semibold text-center">
                            {currentQuestionData?.result?.communication_analysis?.average_pitch['value']}{' '}
                            Hz
                        </p>
                        <p  className="text-center text-sm mt-2">{currentQuestionData?.result?.communication_analysis?.average_pitch[
                                    'category'
                                ]}</p>
                    </div>
                    <div className="w-1/3 border-l-[0.5px] border-gray-300 py-6 grid place-content-center">
                ...
                <div className="flex">
                    ...
                    <div className="flex-1 grid place-content-center p-4 border-l-[0.75px] border-gray-300">
                        <p className="text-4xl text-primary font-bold">
                            {currentQuestionData?.result?.communication_analysis?.average_tone['value']} Hz
                        </p>
                        <p className="text-center text-sm mt-2">(  {
                                currentQuestionData?.result?.communication_analysis?.average_tone['category']   
                            })</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                    <ScoreCard
                        ...
                        value={
                            currentQuestionData?.result?.communication_analysis?.pronunciation_score['value']
                        }
                    />
                    <ScoreCard
                        label="Fluency Score"
                        ...
                        value={currentQuestionData?.result?.communication_analysis?.fluency_score['value']}
                    />
                </div>
                <div className="flex gap-2">
                    <ScoreCard
                        ...
                        value={
                            currentQuestionData?.result?.communication_analysis?.vocabulary_score['value']
                        }
                    />
                    <ScoreCard
                        label="Grammar Score"
                        ...
                        value={currentQuestionData?.result?.communication_analysis?.grammar_score['value']}
                    />
                </div>
            </div>
