...
const CustomLineChart: React.FC<CustomLineChartProps> = ({ type }) => {
    ...
    const generateData = () => {
        ...
        for (var i = 0; i < data?.question_wise_result?.length; i++) {
            ...
            const questionNumber = data?.question_wise_result[i]?.question_id.split('-')[1];

            if (type === 'wpm') {
                const wpm = data?.question_wise_result[i]?.communication_analysis?.['word per min'];
                ...
                const confidenceScore =
                    data?.question_wise_result[i]?.result?.interview_analysis?.confidence_score;
        ...
        if (type === 'wpm')
            return (
                <div className="border-primary border-[1.5px] p-3 bg-white text-xs rounded-md max-w-[200px]">
                    <p>
                        On Question: {payload[0]?.payload?.questionNumber}, your have spoken{' '}
                        <span className="text-primary">{payload[0].value} WPM.</span>
                    </p>
                </div>
            );
        else {
            const confidenceScore = payload[0].value;
            return (
                <div className="border-primary border-[1.5px] p-3 bg-white text-xs rounded-md max-w-[200px]">
                    <p>
                        On Question: {payload[0]?.payload?.questionNumber}, your confidence score
                        was <span className="text-primary">{confidenceScore}</span>.
                    </p>
                </div>
            );
        }
