...
interface TestContextType {
    ...
    setCurrentQuestionData: React.Dispatch<React.SetStateAction<any>>;
}
...
const TestProvider = ({ children }: TestProviderProps): JSX.Element => {
    ...
    const fetchTest = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await axios(`/vocabulary/assessment/${id}/questions/all`);
            if (res.status === 200) {
                setData(res.data);
    ...
    useEffect(() => {
        if (data) {
            setCurrentQuestionData(data?.questions?.[currentQuestionIndex]);
        }
