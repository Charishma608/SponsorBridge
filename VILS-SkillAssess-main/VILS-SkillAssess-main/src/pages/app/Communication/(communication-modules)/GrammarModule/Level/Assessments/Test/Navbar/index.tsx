...
const Navbar = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const response = await axiosV1.post(`/grammar/assessment/${testId}/Submit`, body);
    ...
    return (
        <>
            <div className="border-b-2 p-2 px-4 border-2 rounded-md hidden md:flex items-center justify-between">
                <p className="text-primary font-semibold">Grammar Test</p>
            ...
            <div className="md:hidden">
                <div className="border-b-2 p-2 px-4 border-2 gap-2 rounded-md md:hidden flex items-center justify-between">
                    ...
                    <div className="flex gap-1">
                        <Button
                            label="Exit Test"
                            className="bg-red-600 text-xs md:text-sm lg:px-8 w-fit rounded-md"
                            onClick={() => {
                                setExitButtonPressed(true);
                            }}
                        />
                        <Button
                            label="Submit Test"
                            className="bg-green-600 text-xs md:text-sm  lg:px-8 w-fit rounded-md"
                            onClick={() => {
                                setSubmitButtonPressed(true);
                            }}
                        />
