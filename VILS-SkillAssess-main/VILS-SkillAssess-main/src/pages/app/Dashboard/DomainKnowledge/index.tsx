...
import LineChart from './TestAnalytics/LineChart';
...
const DomainKnowledge = () => {
    ...
    const fetchReports = useCallback(async () => {
        try {
            setIsLoading(true);
            const res = await axiosV2.post(`/dashboard/domain-knowledge`, timeLine);
            if (res.status === 201) {
                setData(res.data);
            }
        } catch (error) {
    ...
    if (!data)
        return (
            <NoDataScreen
                ...
                helpers='Click "Get Started" to take our test and gain insights into your domain expertise, along with personalized suggestions'
            />
        );
    return (
        <div className="flex flex-col gap-4">
            <div className="w-[300px]">
                <ScoreCard
                    ...
                    value={data?.number_of_tests_taken}
                    icon={TestsTaken}
                />
            </div>
            <div className="flex flex-col gap-4">
                        ...
                        <div className="w-[40%]">
                            ...
                            <div className="mt-4 flex flex-col gap-2">
                                <CircularProgressBarGraph
                                    value={Math.round(
                                        parseFloat(data?.assessment_analysis?.avg_score),
                                    )}
                                    label={
                                        Math.round(
                                            parseFloat(data?.assessment_analysis?.avg_score),
