...
const VideoScreen = ({ data, testId }: Props) => {
    ...
    const [stopButtonCounter, setStopButtonCounter] = useState(60);
    ...
    const handleSubmitTest = async () => {
        ...
        try {
            ...
            const res = await axiosV2.post(`/mock-interview/assessment/${testId}/submit`, payload);
            if (res.status === 201) {
            ...
            console.log(error);
            // showAlert('ओह नहीं! 😱 कुछ गलत हो गया। कृपया अधिक विवरण के लिए कंसोल जांचें 🛠️');
            showAlert(
                'Oops! 😱 Something went wrong. Please check the console for more details. 🛠️',
            );
        }
    };
            ...
            <div className="h-screen w-screen overflow-hidden flex flex-col">
                <div className="w-full bg-white shadow-xl">
                    <div className="p-4 border-b flex items-center justify-between">
                        <h1 className="text-primary font-bold text-xl">Reading Test</h1>
                ...
                <div className="w-full flex-1 p-4 overflow-y-auto">
                    {/* <p className='text-xl font-bold'>नीचे दिए गए गद्यांश को पढ़ें:</p> */}
                    <p className="text-xl font-bold">Read the passage written below</p>
                    <p className="mt-4">{data?.questions?.[0]?.question}</p>
                </div>
                <div className="w-full flex items-end gap-5 p-4 border-t-2">
                    {recordingState !== 'TIME_OVER' && recordingState !== 'STOPPED' && (
                        <Button
                            label={
                                recordingState === 'IDLE'
                                    ? 'Start Answering'
